    ...
    public static void printfileInfo(MultipartFile file) {
        ...
        System.out.println("""
                ...
                """.formatted(
                        ...
                        file.getSize() + " byte",   // 용량 (byte)
                        file.getContentType()   // content Type (MIME type)
                ));

        // 이미지 파일 여부
        try {
            BufferedImage bufferedImage = ImageIO.read(file.getInputStream());  // InputStream 으로부터 이미지를 읽어오기
                                                                                // 이미지가 아니면 null 을 리턴
            if(bufferedImage != null){
                System.out.println("""
                            이미지 파일입니다: %d x %d
                        """.formatted(bufferedImage.getWidth(), bufferedImage.getHeight()));    // 가로, 세로 이미지 출력
            } else {
